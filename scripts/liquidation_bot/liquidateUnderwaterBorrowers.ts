...
async function attemptLiquidationViaOnChainLiquidator(
  ...
  console.log( `Attempting to liquidate ${targetAddresses} via OnChainLiquidator @${liquidatorAddress}`);
  ...
  try {
    ...
    if (success) {
      console.log( `Successfully liquidated ${targetAddresses} via ${liquidatorAddress}`);
    } else {
      console.error( `Failed to liquidate ${targetAddresses} via ${liquidatorAddress}`);
    }
    return success;
  } catch (e) {
    console.error(
